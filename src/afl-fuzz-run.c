  ...
  while ((sd_ent = readdir(sd))) {
    ...
    for (o = m; o < n; o++) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        afl->queued_imported +=
            save_if_interesting(afl, mem, st.st_size, mem, st.st_size, fault);
        afl->syncing_party = 0;
...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  u8 fault;
  mutate_helper_init();
  size_t mutated_size = wasm_test(out_buf, len);
  if(!mutated_size){
	  mutate_helper_free();
	  return 1;
  }
  u8 * mutated_out =  mutate_helper_buffer();
   if (unlikely(len = write_to_testcase(afl, (void **)&mutated_out, mutated_size, 0)) == 0) {
  ...
  afl->queued_discovered += save_if_interesting(afl, out_buf, len, mutated_out, mutated_size, fault);
